...
jQuery(document).ready(function ($) {
  ...
  $("#contactForm").validate({
    ...
    submitHandler: function (form) {
      if ($("#contactForm").data("requestRunning")) return false;
      $.ajax({
  ...
  // Back to top button
  $(document)
    .off("click", "#language a")
    .on("click", "#language a", function (e) {
      e.preventDefault();
  ...
  if ($("#nav-menu-container").length) {
    ...
    var $web_oishii = $("#oishii-web")
      .clone()
      .removeClass()
      .wrap("<li></li>")
      .parent();
    var $app_simba = $("#simba-app")
      .clone()
      .removeClass()
      .wrap("<li></li>")
      .parent();
  ...
  $("#form_search").validate({
    ...
    submitHandler: function (form) {
      ...
      if (result["category"] == 0 && result["q"] == "") {
        var placeholder = $("input[name=q]", $(form)).attr("placeholder");
        alert(placeholder);
        return false;
      }
      form.submit();
  ...
  $(document)
    .off("click", ".nav-menu a, #mobile-nav a, .scrollto")
    .on("click", ".nav-menu a, #mobile-nav a, .scrollto", function () {
      if (
  ...
  $("#advanced_comment").click(function (e) {
    ...
    var $role_feedback = $("#advanced_comment").attr("role_feedback")
      ? $("#advanced_comment").attr("role_feedback").split(",")
      : [];
    if (
      $is_logged &&
      ($role_user == "1" || $.inArray($role_user, $role_feedback) != -1)
    ) {
      if ($("#comment-modal .modal-body").is(":empty"))
        $("#comment-modal .modal-body").load(
          path + "ajax/modalfeedback",
          function (result) {
            ...
            $("#gop_y_khac").validate({
              ...
              submitHandler: function (form) {
                if ($("#gop_y_khac").data("requestRunning")) return false;
                $.ajax({
  ...
  $("#logbook").click(function (e) {
    ...
    var $is_logged = $(".logged").length;
    var $role_user = $(".logged").attr("role");
    var $role_feedback = $("#logbook").attr("role_feedback")
      ? $("#logbook").attr("role_feedback").split(",")
      : [];
    if (
      $is_logged &&
      ($role_user == "1" || $.inArray($role_user, $role_feedback) != -1)
...
function download_file(...param) {
  ...
  if (
    role_user &&
    (role_download == "0" ||
      $.inArray(role_user, role_download.split(",")) != -1)
  )
    location.href = path + "ajax/downloadfile?id=" + id;
  else alert(alert_406);
}
